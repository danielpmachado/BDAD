...
-- Table: BASE
CREATE TABLE BASE (NOME VARCHAR, AREA INTEGER CHECK (AREA > 10000), CAPACIDADEMAX INTEGER, VERTENTE VARCHAR REFERENCES RAMOFORCASARMADAS (VERTENTE), LOCAL VARCHAR, PRIMARY KEY (NOME, LOCAL));
...
CREATE TABLE QUANTIDADEVEICULO (IDVEICULO INTEGER, IDMISSAO INTEGER, NUMERO INTEGER, PRIMARY KEY (IDVEICULO, IDMISSAO));

-- Table: RAMOFORCASARMADAS
CREATE TABLE RAMOFORCASARMADAS (VERTENTE VARCHAR PRIMARY KEY, LEMA VARCHAR, MARCHA VARCHAR, PATRONO VARCHAR, GENERAL VARCHAR, CHECK (VERTENTE = 'Força aérea' or VERTENTE = 'Exército' or VERTENTE = 'Marinha'));
...
-- Table: STOCKARMAS
CREATE TABLE STOCKARMAS (NUMERO INTEGER CHECK (NUMERO > 0), IDARMA INTEGER REFERENCES ARMAS (IDARMA), NOMEBASE STRING REFERENCES BASE (NOME), LOCALBASE VARCHAR REFERENCES BASE (LOCAL), PRIMARY KEY (IDARMA, NOMEBASE, LOCALBASE));

-- Table: STOCKVEICULOS
CREATE TABLE STOCKVEICULOS (IDVEICULO INTEGER REFERENCES VEICULO (IDVEICULO), LOCAL VARCHAR REFERENCES BASE ("NOME "), NUMERO INTEGER CHECK (NUMERO > 0));
